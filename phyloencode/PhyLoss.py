...
class PhyLoss(object):
    ...
    def __init__(self, weights : torch.Tensor, rand_matrix : torch.Tensor, char_type = None, latent_layer_Type = "GAUSS" ):
        ...
        self.batch_rdp_loss     = []
        # loss weights
        self.phy_w, self.char_w, self.aux_w, self.mmd_w, self.vz_w, self.rdp_w  = weights
    ...
    def minibatch_loss(self, x : Tuple, y : Tuple, mask : Tuple):
        ...
        phy_loss, char_loss, aux_loss, mmd_loss, vz_loss, rdp_loss = self.losses(x, y, mask, self.char_type)
        ...
        self._append_minibatch_losses(total_loss, phy_loss, char_loss, aux_loss, mmd_loss, vz_loss, rdp_loss)
    ...
    def append_mean_batch_loss(self):
        ...
        mean_rdp_loss   = torch.mean(torch.stack(self.batch_rdp_loss)).item()

        self._append_epoch_losses(mean_total_loss, mean_phy_loss, mean_char_loss, 
                                  mean_aux_loss, mean_mmd_loss, mean_vz_loss, mean_rdp_loss)
    ...
    def _append_minibatch_losses(self, total_loss, phy_loss, 
        ...
        self.batch_rdp_loss.append(rdp_loss)
    ...
    def _append_epoch_losses(self, total_loss, phy_loss, 
        ...
        self.epoch_rdp_loss.append(rdp_loss)
...
# Loss functions
    def losses(self, pred : torch.tensor, true : torch.tensor, mask : torch.tensor, char_type : str):
        ...
        latent_rdp_loss    = self.rdp_loss(latent_hat, phy, self.rand_matrix, n_pairs = 100) if latent_hat is not None else torch.tensor(0.).to(device)

        return phy_loss, char_loss, aux_loss, latent_mmd_loss, latent_vz_loss, latent_rdp_loss
